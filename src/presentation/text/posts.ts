...
export const renderPostCreated = (id: string, creationId: string, mediaType?: string, containerStatus?: string): string => [pc.green('post create: done'), `id: ${id}`, `creation_id: ${creationId}`, `media_type: ${mediaType ?? '-'}`, `container_status: ${containerStatus ?? '-'}`].join('\n')
